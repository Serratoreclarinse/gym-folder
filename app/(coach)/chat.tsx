...
export default function CoachChatScreen() {
  ...
  const { messages, loading, sendMessage, editMessage, deleteMessage, myId } = useChat(clientId);
  ...
  const handleSend = async () => {
    ...
    sendPushNotification(clientId, {
      ...
      body: attachment ? (attachment.type === 'image' ? '📷 Photo' : attachment.type === 'video' ? '🎬 Video' : `📎 ${attachment.name}`) : msg,
      data: { type: 'message' },
    });
  };
  ...
  return (
    ...
    >
      ...
      <View style={[s.header, { paddingTop: (insets.top || 0) + 10 }]}>
        {searchActive ? (
          ...
          <>
            ...
            <Pressable
              style={s.headerInfo}
              onPress={() => router.push({ pathname: '/(coach)/client/[id]', params: { id: clientId } } as any)}
            >
              <View style={s.avatar}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={s.headerName} numberOfLines={1}>{clientName}</Text>
                <Text style={s.headerSub}>Tap for info</Text>
              </View>
            </Pressable>
            ...
            <Pressable>
              <View style={[s.menuSheet, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Pressable
                  ...
                  onPress={() => {
                    setMenuVisible(false);
                    router.push({ pathname: '/(coach)/client/[id]', params: { id: clientId } } as any);
                  }}
                >
                  <Ionicons name="person-outline" size={18} color={colors.textPrimary} />
                  <Text style={[s.menuItemText, { color: colors.textPrimary }]}>View Profile</Text>
                </Pressable>
      ...
      {/* ── Input row ── */}
      <View style={[s.inputRow, { backgroundColor: colors.surface, borderTopColor: colors.border, paddingBottom: (insets.bottom || 0) + 12 }]}>
                ...
                <>
                  ...
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      Alert.alert(
                        'Delete for Everyone?',
                        'This message will be removed for both you and your client.',
                        [
...
function makeStyles(c: ColorScheme) {
  return StyleSheet.create({
    ...
    searchInput: {
      flex: 1, fontSize: 15, paddingVertical: 4,
    },
    searchBar: {
    ...
    menuSheet: {
      borderRadius: 10, borderWidth: 1, minWidth: 180,
      overflow: 'hidden',
    },
    ...
    headerName: { ...Typography.body, color: c.textPrimary, fontWeight: '700', fontSize: 16 },
