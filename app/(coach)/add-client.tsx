...
function Field({
  label, value, onChangeText, placeholder, keyboardType, required, error, colors, styles,
}: {
  label: string;
  value: string;
  ...
  return (
    <KeyboardAvoidingView style={styles.kav} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Field label="Full Name" value={name} onChangeText={setName} placeholder="Jane Smith" required colors={colors} styles={styles} />
        <Field
          label="Email"
          value={email}
...
function makeStyles(c: ColorScheme) {
  return StyleSheet.create({
    ...
    hint: { ...Typography.caption, color: c.textSecondary, marginTop: 6 },
    ...
    previewText: { ...Typography.body, color: c.accent, flex: 1 },
    submitBtn: {
