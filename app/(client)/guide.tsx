...
function getSections(c: ColorScheme): Section[] {
  return [
    {
    ...
    },
    {
      ...
      steps: [
        ...
        'Q: I\'m not receiving notifications. What should I do?\nA: Go to your phone\'s Settings → Apps → ELEVATƎ → Notifications → enable "Allow Notifications." Then reopen the app.',
      ],
    },
  ];
}

function GuideCard({ section, s }: { section: Section; s: ReturnType<typeof makeStyles> }) {
  ...
  return (
    <View style={s.card}>
      <Pressable style={s.cardHeader} onPress={() => setOpen((v) => !v)}>
        ...
        <Ionicons name={open ? 'chevron-up' : 'chevron-down'} size={18} color={s.cardSubtitle.color as string} />
      </Pressable>
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={s.content}>
      ...
      {sections.map((section) => (
        <GuideCard key={section.title} section={section} s={s} />
      ))}
